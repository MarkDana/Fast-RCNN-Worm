...
def rel_bbox(size, bbox):#bbox是一组bounding boxes，nroi*4, 4dim is xmin,ymin,xmax,ymax
...
def calc_ious(ex_rois, gt_rois):
    ex_area = (1. + ex_rois[:,2] - ex_rois[:,0]) * (1. + ex_rois[:,3] - ex_rois[:,1])#nroi*1
    gt_area = (1. + gt_rois[:,2] - gt_rois[:,0]) * (1. + gt_rois[:,3] - gt_rois[:,1])#number_of_groundtruth*1
    area_sum = ex_area.reshape((-1, 1)) + gt_area.reshape((1, -1))
    #reshape，行+列 是为了调整到 nroi*number_of_groundtruth shape
    ...
    ious = area_i / area_u #return nroi*number_of_groundtruth的array
    return ious
